...
const validateStore = [
  ...
  "const repo = payload.context.repository;",
  "const pr = payload.context.pullRequest;",
  "const sha = String(payload.context.commitSha);",
  "const SUMMARY = { 'sast.critical_new': 'New critical code security issue', 'sast.high_new': 'New high-severity code security issue', 'dependencies.critical_with_fix': 'Critical vulnerable dependency (fix available)', 'dependencies.high_with_fix': 'High-severity vulnerable dependency (fix available)' };",
  "const summary = [...new Set(payload.findings.map(f => SUMMARY[f.policyRule] || f.policyRule))].map(s => '*' + s + '*').join('\\n');",
  "const findingLink = (f) => { if (f.source !== 'semgrep' || typeof f.location !== 'string') return ''; const i = f.location.lastIndexOf(':'); if (i < 1) return ''; const p = f.location.slice(0, i); const ln = f.location.slice(i + 1); if (!p || !/^\\d+$/.test(ln)) return ''; return '  <https://github.com/' + repo + '/blob/' + sha + '/' + p + '#L' + ln + '|View finding>'; };",
  "const detail = payload.findings.map(f => f.policyRule + ': ' + f.id + ' - ' + f.reason + findingLink(f)).join('\\n').slice(0, 2900);",
  "const expiresTs = Math.floor(new Date(request.expiresAt).getTime() / 1000);",
  "const expires = Number.isFinite(expiresTs) ? '<!date^' + expiresTs + '^{date_short_pretty} {time}|' + request.expiresAt + '>' : request.expiresAt;",
  "const channel = $env.SLACK_CHANNEL_ID;",
  "if (!channel) throw new Error('SLACK_CHANNEL_ID is not configured');",
  "const slackMessage = { channel: channel, text: 'Break-glass security exception requested', unfurl_links: false, unfurl_media: false, blocks: [",
  ...
  "    { type: 'mrkdwn', text: '*Pull request*\\n<https://github.com/' + repo + '/pull/' + pr + '|#' + pr + '>' },",
  "    { type: 'mrkdwn', text: '*Commit*\\n<https://github.com/' + repo + '/commit/' + sha + '|' + sha.slice(0, 12) + '>' },",
...
const verify = [
  ...
  "let interaction;",
  "try {",
  "  // Parse application/x-www-form-urlencoded with core-JS globals only;",
  "  // URLSearchParams is not exposed in the n8n Code-node sandbox.",
  "  const bodyText = rawBody.toString('utf8');",
  "  const m = /(?:^|&)payload=([^&]*)/.exec(bodyText);",
  "  if (!m) throw new Error('no payload');",
  "  const payloadField = decodeURIComponent(m[1].replace(/\\+/g, ' '));",
